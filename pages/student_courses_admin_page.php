...
if(!empty($_GET['action'])){
	if ($_GET['action'] == 'add') {
		require(LMS_PLUGIN_PATH.'admin/student_courses-add.php');
		exit();
	}

	if ($_GET['action'] == 'delete') {
		require(LMS_PLUGIN_PATH.'admin/student_courses-delete.php');
		exit();
	}
}
...
if(!empty($_GET['code'])){
	list($actionTaken, $code) = explode(":", $_GET['code']);

	if($actionTaken == "add")
		$actionTakenStr = "added";
	else if ($actionTaken == "del")
		$actionTakenStr = "deleted";
	else
		$actionTakenStr ="";

	switch(intval($code)){
		case ErrorCodeConstants::ALREADY_EXIST:
			$message = "Course already exist. Not ". $actionTakenStr;
			break;
		case ErrorCodeConstants::FAIL:
			$message = "Course not ". $actionTakenStr;
			break;
		case ErrorCodeConstants::SUCCESS:
			$message = "Course ". $actionTakenStr;
			break;
		default:
			$message = "";
	}
...
$userid = $_GET['userid'];
$studentAdminUrl = LMS_PageFunctions::getAdminUrlPage(STUDENTS_ADMIN_PAGE,is_SSL());
...
<a class="add-new-h2" href="<?php echo $addNewUrl ?>">Add New Courses</a><a class="add-new-h2" href="<?php echo $studentAdminUrl ?>">All Students</a></h2>
	...
	<table class="widefat">
        ...
        <tr>
            <td><?php echo $scourse['course_name']; ?>
                <div class="row-actions"><span class="trash"><a href="<?php echo $deleteUrl."&courseid=".$scourse['course_id']."&ucid=".$scourse["id"]; ?>" onclick='return confirm(" Are you sure! You want to delete this record")'> Delete</a>  </span></div>
            </td>
